...
namespace {
	...
	struct MmlException : public std::runtime_error {
		...
		MmlException(MmlCompiler::ErrorCode code, std::string::const_iterator it, size_t length = 0);
		MmlException(std::vector<MmlCompiler::Result::Error>&& errors);
	};

	// iteratorから行番号と列番号を取得(どちらも1始まり)
	const auto getLineColumn = [](const std::string& text, const std::string::const_iterator& it) -> std::pair<size_t, size_t> {
		size_t line = 1;
		size_t column = 1;
		for (auto i = text.begin(); i != it; i++) {
			if (*i == '\r') {
				line++;
				column = 1;
				if (auto next = std::next(i); next != text.end() && *next == '\n') ++i;	// // 次が '\n' なら飛ばす（CRLF）
			} else if (*i == '\n') {
				line++;
				column = 1;
			} else if ((static_cast<unsigned char>(*i) & 0xc0) != 0x80) {	// UTF-8 継続バイト以外(=文字開始)なら
				column++;
			}
		}
		return { line, column };
	...
	// iteratorからエラー箇所の文字列を取得
	const auto getPositionText = [](const std::string::const_iterator& it, const std::string::const_iterator& itEnd, size_t length = 8) -> std::string {
		std::string result;
		size_t column = 0;
		for (auto i = it; i != itEnd; i++) {
			if (*i == '\r' || *i == '\n') break;
			if ((static_cast<unsigned char>(*i) & 0xc0) != 0x80) {	// UTF-8 継続バイト以外(=文字開始)なら
...
std::string MmlCompiler::Result::getMessage(ErrorCode code) {
	static const std::map<ErrorCode, std::string> m = {
...
std::string MmlCompiler::Result::getText(const std::string& mml, const std::vector<Result::Error>& errors) {
	...
	for (const auto err : errors) {
		const auto lineColum = getLineColumn(mml, err.it);				// 行と列を取得
		const auto msg = MmlCompiler::Result::getMessage(err.code);	// エラーメッセージ取得
		const auto errorText = getPositionText(err.it, mml.cend(), 10);	// エラー箇所のMML
		result += result.empty() ? "" : "\n";
		result += string::format(u8"%d:%d error %03d: %s '%s'", lineColum.first, lineColum.second, static_cast<size_t>(err.code), msg, errorText);
	}
	return result;
}
	...
	for (const auto err : errors) {
		...
		list.push_back(
			Json::Map{
				...
				{"line",	lineColum.first},
				{"column",	lineColum.second},
				{"message",	string::format(u8"%s '%s'", msg, errorText)},
			});
	}
...
namespace {
	...
	std::optional<boost::smatch> regexSearch(std::string::const_iterator iBegin, std::string::const_iterator iEnd, const regex& re) {
	...
	// 文字列リテラルパース
	auto parseString(std::string::const_iterator iBegin, std::string::const_iterator iEnd) {
		struct Result {
	...
	std::string::const_iterator parseComment(std::string::const_iterator iBegin, std::string::const_iterator iEnd) {
		auto it = iBegin;
		while (true) {
			...
			for (; i != list.end(); i++) {
				if (const auto r = isStartsWith(it, iEnd, i->first)) {
					if (const auto s = regexSearch(*r, iEnd, i->second)) {	// コメントの終了位置検索
						...
						throw MmlException(MmlCompiler::ErrorCode::commentError, it, 1);	// 見つからないならエラー
					}
				}
			}
	...
	// 整数パース
	auto parseInt(std::string::const_iterator iBegin, std::string::const_iterator iEnd) {
		struct {
	...
	// 浮動小数点数パース
	auto parseDouble(std::string::const_iterator iBegin, std::string::const_iterator iEnd) {
		struct {
			std::string::const_iterator next;
			double						value;
		}result;
		...
		while (true) {
				...
				if (const auto r = regexSearch(it, iEnd, re)) {
					...
					if (*next == ':') {													// ":" があれば引数名で確定
						...
						if (auto i = argNames.find(currentArgName); i == argNames.end()) {	// 引数名チェック
							throw MmlException(MmlCompiler::ErrorCode::argumentUnknownError, it);
						}
						it = next + 1;
						flags.all = 0;
						flags.argValue = true;
			...
			if (flags.argValue) {		// 引数値
				...
				if (auto r = MmlCompiler::Util::parseWord(it, iEnd)) {
					if (!r->word) {			// パースエラー
						throw MmlException(MmlCompiler::ErrorCode::argumentError, it);	// 文字列パースエラーとする
					}
					it = r->next;
					if (currentArgName.empty()) {
						result->argsList.emplace_back(std::move(*r->word));
						if (result->argsList.size() > argCount) {
							throw MmlException(MmlCompiler::ErrorCode::argumentError, it);	// 引数が多すぎる
						}
					} else {
				...
				throw MmlException(MmlCompiler::ErrorCode::argumentError, it);
			}

			break;		// どれにも該当しないならエラー
		}
		throw MmlException(MmlCompiler::ErrorCode::functionCallError, it);
	};

	// 音長解析
	auto parseLength(std::string::const_iterator iBegin, std::string::const_iterator iEnd, size_t defaultLength) {
		struct Result {
			std::string::const_iterator	next;	// 次の位置
			size_t	step = 0;
		}result = { iBegin };
		...
		while (true) {
			...
			if (auto r = regexSearch(it, iEnd, regex(R"(^([0-9]+))"))) {
				...
				if (stepNotation) {		// ステップ数指定しておきながら数値がないなら
					throw MmlException(MmlCompiler::ErrorCode::lengthError, it);
				}
			}
		...
		if (step < 0) {		// 負値はエラー
			throw MmlException(MmlCompiler::ErrorCode::lengthMinusError, iBegin);
		}
		result.step = step;
		return result;
	}
...
public:
	...
	static std::vector<Port> mmlToSequence(std::string::const_iterator iBegin, std::string::const_iterator iEnd, const Sequences& sequences) {
		struct PortInfo {
		...
		// ^ tie (長さを付け足す)
		const auto parseTie = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
		...
		// < > オクターブUPDOWN
		const auto parseOctaveUpDown = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			auto& port = *state.currentPort;
			const auto check = [&] {
				if (port.octave < -2 || port.octave > 8) {	// 範囲チェック
					throw MmlException(ErrorCode::octaveUpDownRangeCommandError, begin);
				}
			};
		...
		// ' Noteで現在位置を進めるか否かモード
		const auto parseNoteUnmove = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (const auto r = isStartsWith(begin, end, "'")) {
				auto& port = *state.currentPort;
		...
		// @?? プログラムチェンジ
		const auto parseProgramChange = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (const auto r = isStartsWith(begin, end, "@")) {
				auto it = parseComment(*r, end);			// コメントを読み飛ばす
				...
				throw MmlException(ErrorCode::programchangeCommandError, begin);
			}
			return std::nullopt;
		};

		const auto parseCreateSequence = [&state, &sequences](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (auto r = parseFunction(begin, end, { "CreateSeq","CreateSequence" }, { "name","mml" })) {
				const auto name = (*r).findArgString("name").value_or("");
				if (name.empty()) {
					throw MmlException(ErrorCode::createSequenceNameError, begin);
				}
				if (const auto i = state.sequences.find(name); i != state.sequences.end()) {	// 既にあるなら
					throw MmlException(ErrorCode::createSequenceDuplicateError, begin);
				}
				const auto mml = (*r).findArg<PairIterator>("mml").second;
				if (!mml) {
					throw MmlException(ErrorCode::createSequenceError, begin);
				}
				Sequence seq;
				seq.name = name;
		...
		const auto parseSequence = [&state, &sequences](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (auto r = parseFunction(begin, end, { "Seq","Sequence" }, { "length" })) {
				const auto name = (*r).findArgString(0).value_or("");
				if (name.empty()) {
					throw MmlException(ErrorCode::sequenceNameError, begin);
				}
				...
				seq.length = [&]()->std::optional<size_t> {
					...
					if (!arg.second) throw MmlException(ErrorCode::sequenceLengthError, begin);	// 型が違う
					...
					if (len.next != length->second) {
						throw MmlException(ErrorCode::sequenceLengthError, begin);		// 余計な文字列がある
					}
					return len.step;
				}();
		...
		const auto parseCreatePort = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (const auto r = parseFunction(begin, end, { "CreatePort","createPort" }, { "name","instrument","channel" })) {		// CreatePort
				const auto name = (*r).findArgString("name").value_or("");
				if (name.empty()) {
					throw MmlException(ErrorCode::createPortPortNameError, begin);
				}
				auto r2 = state.mapPort.insert({ name ,PortInfo() });	// port追加
				if (!r2.second) {				// 既にあるなら
					throw MmlException(ErrorCode::createPortDuplicateError, begin);
				}
				...
				if (!ch || *ch < 1 || *ch > 16) {
					throw MmlException(ErrorCode::createPortChannelError, begin);
				}
		...
		const auto parsePort = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (auto r = parseFunction(begin, end, { "Port","port" }, {})) {		// Port
				const auto name = (*r).findArgString(0).value_or("");
				if (name.empty()) {
					throw MmlException(ErrorCode::portNameError, begin);
				}
		...
		const auto parseVolume = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (auto r = parseFunction(begin, end, { "V","Volume","volume" }, {})) {		// Volume
				...
				if (const auto v = r->findArg<intmax_t>(0).second) {	// 符号付なら相対指定
					...
					if (*v < 0 || *v > 127) {
						throw MmlException(ErrorCode::volumeRangeError, begin);
					}
					port.volume = static_cast<decltype(port.volume)>(*v);
				} else {
					throw MmlException(ErrorCode::volumeError, begin);
				}
		...
		const auto parsePan = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (auto r = parseFunction(begin, end, { "Pan","pan" }, {})) {		// Pan
				...
				if (const auto v = r->findArg<intmax_t>(0).second) {	// 符号付なら相対指定
					...
					if (*v < 0 || *v > 127) {
						throw MmlException(ErrorCode::panRangeError, begin);
					}
					port.pan = static_cast<decltype(port.pan)>(*v);
				} else {
					throw MmlException(ErrorCode::panError, begin);
				}
		...
		const auto parsePitchBend = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (auto r = parseFunction(begin, end, { "PitchBend","pitchBend" }, {})) {		// PitchBend
				const auto v = r->findArgInt(0);
				if (!v) throw MmlException(ErrorCode::pitchBendError, begin);
				auto& port = *state.currentPort;
				if (*v < -8192 || *v > 8191) {
					throw MmlException(ErrorCode::pitchBendRangeError, begin);
				}
		...
		const auto parseControlChange = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (auto r = parseFunction(begin, end, { "CC","ControlChange","controlChange" }, { "no","value" })) {		// ControlChange
				const auto no = [&] {
					...
					throw MmlException(ErrorCode::controlChangeError, begin);
				}();
				const auto val = [&] {
				...
				if (no < 0 || no > 127 || val < 0 || val > 127) {
					throw MmlException(ErrorCode::controlChangeRangeError, begin);
				}
				auto& port = *state.currentPort;
		...
		// t?? テンポ
		const auto parseTempo = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (const auto r = isStartsWith(begin, end, "t")) {
				...
				throw MmlException(ErrorCode::tCommandRangeError, begin);
			}
			return std::nullopt;
		};

		// r?? 休符
		const auto parseRest = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
		...
		// a～g?? 音符
		const auto parseNote = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			...
			e->note = [&] {
				...
				throw MmlException(ErrorCode::noteCommandRangeError, begin);
			}();
			e->length = len.step;
		...
		// l?? デフォルト音長
		const auto parseDefaultLength = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
		...
		// o?? オクターブ
		const auto parseOctave = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (const auto r = isStartsWith(begin, end, "o")) {
				...
				if (const auto r2 = regexSearch(it, end, re)) {
					...
					if (octave < -2 || octave > 8) {		// 範囲チェック
						throw MmlException(ErrorCode::oCommandRangeError, begin);
					}
					port.octave = octave;
					port.beforeEvent.reset();				// '^'の対象をクリア
					return (*r2)[0].second;
		...
		// v? ベロシティ
		const auto parseVelocity = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (const auto r = isStartsWith(begin, end, "v")) {
				auto it = parseComment(*r, end);			// コメントを読み飛ばす
				const auto v = parseInt(it, end);
				if (!v) throw MmlException(ErrorCode::vCommandError, begin);
				auto& port = *state.currentPort;
				if (std::holds_alternative<intmax_t>(v->value)) {// 符号付きなら相対指定
					...
					if (n < 0 || n > 127) {		// 範囲チェック
						throw MmlException(ErrorCode::vCommandRangeError, begin);
					}
		...
		// メタイベント
		const auto parseMeta = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (auto r = parseFunction(begin, end, { "Meta" }, { "type" })) {
				...
				if (!type || *type < 0 || *type > std::numeric_limits<uint8_t>::max()) {
					throw MmlException(ErrorCode::metaTypeError, begin);
				}
				auto& port = *state.currentPort;
				...
				for (auto& arg : r->argsList) {
					if (std::holds_alternative<uintmax_t>(arg)) {
						...
						throw MmlException(ErrorCode::metaTypeError, begin);	// failsafe
					}
				}
		...
		// FineTune
		const auto parseFineTune = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (auto r = parseFunction(begin, end, { "FineTune" }, {})) {
				const auto v = r->findArgNumber(0);
				if (!v) throw MmlException(ErrorCode::fineTuneError, begin);
				auto& port = *state.currentPort;

				constexpr double inMin = -100.0;
				constexpr double inMax = 100.0;
				if (*v < inMin || *v > inMax) {
					throw MmlException(ErrorCode::fineTuneRangeError, begin);
				}
		...
		// CoarseTune
		const auto parseCoarseTune = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (auto r = parseFunction(begin, end, { "CoarseTune" }, {})) {
				const auto v = r->findArgInt(0);
				if (!v) throw MmlException(ErrorCode::coarseTuneError, begin);
				auto& port = *state.currentPort;
				if (*v < -64 || *v > 63) {
					throw MmlException(ErrorCode::coarseTuneRangeError, begin);
				}
				struct {
					midi::EventControlChange::Type no;
					uint8_t val;
		...
		const auto parseDefinePresetFM = [&state](const_iterator begin, const_iterator end)->std::optional<const_iterator> {
			if (auto r = parseFunction(begin, end, { "DefinePresetFM" }, { "no","name" }, 38)) {
				const auto no = (*r).findArgInt("no");
				if (!no || *no < 0 || *no>127) {
					throw MmlException(ErrorCode::definePresetFMNoError, begin);
				}
				const auto name = (*r).findArgString("name").value_or("");
				...
				for (size_t i = 0; i < maxTable.size(); i++) {
					...
					if (!n) throw MmlException(ErrorCode::definePresetFMError, begin);
					if (*n<0 || *n> maxTable[i]) throw MmlException(ErrorCode::definePresetFMRangeError, begin);
		...
		const std::initializer_list<
			std::pair<std::function<std::optional<const_iterator>(const_iterator, const_iterator)>, ErrorCode>
		> funcs = {
			{parseTie,				ErrorCode::tieCommandError},			// ^ tie (長さを付け足す)
			{parseRest,				ErrorCode::rCommandRangeError},		// r?? 休符
			{parseOctaveUpDown,		ErrorCode::octaveUpDownCommandError},	// < > オクターブUPDOWN
			{parseNoteUnmove,		ErrorCode::unknownError},				// ' noteで位置更新するか否かモード
			{parseProgramChange,	ErrorCode::programchangeCommandError},// @? プログラムチェンジ
			{parseTempo,			ErrorCode::tCommandRangeError},		// t?? テンポ
			{parseDefaultLength,	ErrorCode::lCommandError},			// l?? デフォルト音長
			{parseOctave,			ErrorCode::oCommandError},			// o?? オクターブ
			{parseCreatePort,		ErrorCode::createPortError},			// CreatePort
			{parsePort,				ErrorCode::portError},				// Port
			{parseVolume,			ErrorCode::volumeError},				// Volume
			{parsePan,				ErrorCode::panError},					// Pan
			{parsePitchBend,		ErrorCode::pitchBendError},			// PitchBend
			{parseControlChange,	ErrorCode::controlChangeError},		// ControlChange
			{parseNote,				ErrorCode::noteCommandRangeError},	// a～g?? 音符
			{parseVelocity,			ErrorCode::vCommandError},			// v? ベロシティ
			{parseCreateSequence,	ErrorCode::createSequenceError},		// CreateSequence
			{parseSequence,			ErrorCode::sequenceError},			// Sequence
			{parseMeta,				ErrorCode::metaError},				// Meta
			{parseFineTune,			ErrorCode::fineTuneError},			// FineTune
			{parseCoarseTune,		ErrorCode::coarseTuneError},			// CoarseTune
			{parseDefinePresetFM,	ErrorCode::definePresetFMError}		// DefinePresetFM
		};
			...
			try {
				...
				for (; i != funcs.end(); i++) {
					try {
						...
						throw MmlException(i->second, it);
					}
				}
				if (i == funcs.end()) throw MmlException(ErrorCode::unknownError, it);	// 未定義文字列エラー
			} catch (MmlException& e) {
				errors.insert(errors.end(), e.errors.begin(), e.errors.end());
				it = std::find_if(it, iEnd, [](char c) {return c == '\n' || c == '\r'; });	// 行末まで飛ばす
			}
		}
...
MmlException::MmlException(MmlCompiler::ErrorCode code, std::string::const_iterator it, size_t length)
	:std::runtime_error(MmlCompiler::Result::getMessage(code))
	, errors({ {code,it,length} })
{}

MmlException::MmlException(std::vector<MmlCompiler::Result::Error>&& errors)
...
MmlCompiler::Result MmlCompiler::compile(const std::string& mml) {
	...
	try {
		...
		r.ports = Inner::mmlToSequence(mml.cbegin(), mml.cend(), sequences);
	} catch (MmlException& e) {
...
std::optional<MmlCompiler::Util::ParsedWord> MmlCompiler::Util::parseWord(std::string::const_iterator iBegin, std::string::const_iterator iEnd) {
