...
MML を 標準MIDIファイル へコンパイル(変換)するクラスライブラリです。C++20 での実装です。
...
|Ep([値])<br>別名 Expression|エクスプレッションです。値は 0～127です。<br>符号を付けると現在値からの相対指定になります。| "Ep(120) cde Ep(-30) fga"<br>→ エクスプレッション120でドレミです、エクスプレッション90でファソラです|
...
|MasterVolume([値])|マスターボリュームです。値は 0～16383 です<br>システムエクスクルーシブのGMマスターボリュームの別名です| // マスターボリュームを10000にします<br/>MasterVolume(10000)|
|SysEx(<br>&emsp;[データ]...<br>)<br>|システムエクスルーシブです。<br>名前ナシの可変長引数でデータを指定します。先頭バイトは 0xf0、0xf7 のいずれかを、末尾は（原則として）0xf7 を指定します| // Roland GS 音源をリセットします（GS Reset）<br/>SysEx(0xf0,0x41,0x10,0x42,0x12,0x40,0x00,0x7f,0x00,0x41,0xf7)|
